...
#
# Usage:
#   ./radar_info.sh [-k] [http://localhost:6502]
#
# Options:
#   -k  Allow insecure TLS connections (self-signed certificates)
#
...
if ! "${CURL[@]}" -f "${BASE_URL}/signalk" >/dev/null 2>&1; then
    if [[ "${BASE_URL}" == https://* ]]; then
        echo "Error: Cannot reach ${BASE_URL}. If using a self-signed certificate, pass -k as the first argument." >&2
    else
