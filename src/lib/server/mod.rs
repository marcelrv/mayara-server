...
pub(crate) mod axum_fix;
pub(crate) mod recordings;
pub(crate) mod signalk;
...
impl Web {
    ...
    /// Android-specific variant: binds to `127.0.0.1` only (IPv4 loopback), no TLS.
    ///
    /// Called by `start_android()`. Does not require socket2 dual-stack setup.
    pub async fn run_android(self, subsys: SubsystemHandle) -> Result<(), WebError> {
        let port = self.args.port;
        ...
        let app = router.into_make_service();
        log::info!(
            "Starting HTTP web server (Android, loopback only) on port {} (pid {})",
            port,
            std::process::id()
        );
...
/// Start the full mayara radar server for Android (embedded mode).
///
/// Binds only to `127.0.0.1:{cli.port}` (loopback, no external network exposure).
/// The `shutdown_rx` oneshot is provided by the JNI layer; when it fires, the server
/// performs a graceful 5-second shutdown and all subsystems stop cleanly.
///
/// # Errors
/// Returns an error if the Tokio subsystem hierarchy fails to start or shut down.
pub async fn start_android(
    ...
    Toplevel::new(|s| async move {
        let web = Web::new(&s, cli).await;

        // Forward JNI oneshot shutdown signal → Toplevel graceful shutdown.
        s.start(SubsystemBuilder::new("JniShutdownWatcher", |sub| async move {
            tokio::select! {
                _ = shutdown_rx => { sub.request_shutdown(); }
                _ = sub.on_shutdown_requested() => {}
            }
            Ok::<(), anyhow::Error>(())
        }));

        s.start(SubsystemBuilder::new("Webserver", move |a| web.run_android(a)));
    })
